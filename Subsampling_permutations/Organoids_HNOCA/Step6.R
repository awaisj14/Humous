...
# ---------------- CLI ARGS (perm or "avg") ----------------
args <- commandArgs(trailingOnly = TRUE)
cli_tag <- if (length(args) >= 1) args[1] else NA_character_
# valid examples: "1", "07", "avg"
if (!is.na(cli_tag)) {
  if (!identical(tolower(cli_tag), "avg")) {
    if (suppressWarnings(is.na(as.integer(cli_tag)))) {
      stop("Usage: Rscript Step8_corr.R <perm_id|avg>\n",
           "  e.g. Rscript Step8_corr.R 1\n",
           "       Rscript Step8_corr.R avg")
    }
  }
}
...
# Robust loader for perm/avg landscape (paths match make_all_perm_pngs.R)
...
if (length(all_summaries)) {
  ...
  if (file.exists(summ_path)) {
    # append without header if same columns; otherwise overwrite
    try({
      old <- suppressWarnings(readr::read_csv(summ_path, show_col_types = FALSE))
      if (setequal(names(old), names(summ))) {
        out <- bind_rows(old, summ)
        out <- distinct(out)  # de-dup if you re-run same perm
        write_csv(out, summ_path)
      } else {
